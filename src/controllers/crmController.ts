...
export const deleteDealsAndSubDependencies = async (supabase: any, dealIds: string[]) => {
  ...
  if (contractIds.length > 0) {
    // Desvincular rental_invoice_equipments ligados a esses contratos (não excluir da locação)
    await supabase.from('rental_invoice_equipments').update({ deal_contract_id: null }).in('deal_contract_id', contractIds);
  ...
  try {
    ...
    if (lastContract?.snapshot) {
      ...
      return res.json({
        ...
        billing_interval_days: s.billing_interval_days || '28 dias',
...
export const buildContractSnapshot = async (form: any, contractNumber: string) => {
  ...
  return {
    ...
    billing_interval_days: form.billing_interval_days || '28 dias',
